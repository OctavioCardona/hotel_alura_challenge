...
public class ReservasView extends JFrame {
	...
	private void guardarReservas() {
		String DateIn = ((JTextField)txtFechaEntrada.getDateEditor().getUiComponent()).getText();
		String DateOut = ((JTextField)txtFechaSalida.getDateEditor().getUiComponent()).getText();
		Reservas nuevaReservacion = new Reservas(
				java.sql.Date.valueOf(DateIn),
				java.sql.Date.valueOf(DateOut),
				txtValor.getText(),
				txtFormaPago.getSelectedItem().toString());
		reservasController.guardar(nuevaReservacion);
		JOptionPane.showMessageDialog(contentPane, "La reservacion se guardo de manera exitosa, id: " + nuevaReservacion.getId());
		RegistroHuesped huesped = new RegistroHuesped(nuevaReservacion.getId());
	...
	private void calcularValor(JDateChooser fechaEntrada, JDateChooser fechaSalida) {
		if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {		
			...
			Long entrada = java.sql.Date.valueOf(fechaE).getTime();
			Long salida = java.sql.Date.valueOf(fechaS).getTime();
			Long diferencia = salida - entrada;
			double diasEstancia = Math.floor(diferencia / (1000 * 60 * 60 * 24));
			System.out.println(diasEstancia);
			double precioEstancia = 20 * diasEstancia;
			System.out.println(precioEstancia);
			txtValor.setText(String.valueOf(precioEstancia));
		}
	}
